...
class WpAdvQuiz_Model_StatisticRefMapper extends WpAdvQuiz_Model_Mapper
{
    ...
    public function fetchAll($quizId, $userId, $testId = 0)
    {
        ...
        foreach ($results as $row) {
            ...
            $r[] = new WpAdvQuiz_Model_StatisticRefModel($row);
        }
    ...
    public function fetchByRefId($refIdUserId, $quizId, $avg = false)
    {
        ...
        foreach ($results as $row) {
            ...
            return new WpAdvQuiz_Model_StatisticRefModel($row);
        }
    ...
    public function fetchAvg($quizId, $userId)
    {
        ...
        foreach ($results as $row) {
            $r[] = new WpAdvQuiz_Model_Statistic($row);
        }
    ...
    public function fetchOverview($quizId, $onlyCompleded, $start, $limit)
    {
        ...
        foreach ($results as $row) {
            ...
            $a[] = new WpAdvQuiz_Model_StatisticOverview($row);
        }
    ...
    /**
     *
     * @param WpAdvQuiz_Model_StatisticRefModel $statisticRefModel
     * @param WpAdvQuiz_Model_Statistic[] $statisticModel
     */
...
// 		}
    ...
    public function fetchFormOverview($quizId, $page, $limit, $onlyUser = 0)
    {
        ...
        foreach ($result as $row) {
            ...
            $r[] = new WpAdvQuiz_Model_StatisticFormOverview($row);
        }
     ...
     * @param int $startTime
     * @param int $endTime
     * @return WpAdvQuiz_Model_StatisticHistory[]
     */
    public function fetchHistory($quizId, $page, $limit, $users = -1, $startTime = 0, $endTime = 0)
    {
        ...
        foreach ($result as $row) {
            ...
            $r[] = new WpAdvQuiz_Model_StatisticHistory($row);
        }
    ...
    protected function fetchStatisticFromUserList($userData, $quizId, $start, $limit)
    {
        ...
        foreach ($userData as $user) {
            if (isset($statistics[$user->ID])) {
                ...
                $a[] = new WpAdvQuiz_Model_StatisticOverview($statistics[$user->ID]);
            } else {
                $a[] = new WpAdvQuiz_Model_StatisticOverview([
                    'user_id' => $user->ID,
