...
class WpAdvQuiz_Model_ToplistMapper extends WpAdvQuiz_Model_Mapper
{
    ...
    public function save(WpAdvQuiz_Model_Toplist $toplist)
    {
     ...
     * @param $sort
     * @param int $start
     * @return WpAdvQuiz_Model_Toplist[]
     */
    public function fetch($quizId, $limit, $sort, $start = 0)
    {
        ...
        switch ($sort) {
            case WpAdvQuiz_Model_Quiz::QUIZ_TOPLIST_SORT_BEST:
                $s = 'ORDER BY result DESC';
                break;
            case WpAdvQuiz_Model_Quiz::QUIZ_TOPLIST_SORT_NEW:
                $s = 'ORDER BY date DESC';
                break;
            case WpAdvQuiz_Model_Quiz::QUIZ_TOPLIST_SORT_OLD:
            default:
                $s = 'ORDER BY date ASC';
                break;
        }
        ...
        foreach ($results as $row) {
            $r[] = new WpAdvQuiz_Model_Toplist($row);
        }
