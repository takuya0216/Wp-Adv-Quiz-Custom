...
class WpAdvQuiz_Model_Question extends WpAdvQuiz_Model_Model
{
    protected $_id = 0;
    protected $_quizId = 0;
    protected $_sort = 0;
     ...
     * @return WpAdvQuiz_Model_AnswerTypes[]|null|string
     */
    public function getAnswerData($serialize = false)
    {
        ...
        if (is_array($this->_answerData) || $this->_answerData instanceof WpAdvQuiz_Model_AnswerTypes) {
            if ($serialize) {
                return @serialize($this->_answerData);
            }
        } else {
            if (!$serialize) {
                if (WpAdvQuiz_Helper_Until::saveUnserialize($this->_answerData, $into) === false) {
                    return null;
                }
