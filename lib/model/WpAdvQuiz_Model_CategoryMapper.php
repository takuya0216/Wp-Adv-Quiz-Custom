...
class WpAdvQuiz_Model_CategoryMapper extends WpAdvQuiz_Model_Mapper
{
     ...
     */
    public function fetchAll($type = WpAdvQuiz_Model_Category::CATEGORY_TYPE_QUESTION)
    {
        $type = $type == WpAdvQuiz_Model_Category::CATEGORY_TYPE_QUESTION ? $type : WpAdvQuiz_Model_Category::CATEGORY_TYPE_QUIZ;
        ...
        foreach ($results as $row) {
            $r[] = new WpAdvQuiz_Model_Category($row);
        }
    ...
    public function fetchByQuiz($quizId)
    {
        ...
        $results = $this->_wpdb->get_results($this->_wpdb->prepare('
			...
			ORDER BY
       			c.category_name
		', WpAdvQuiz_Model_Category::CATEGORY_TYPE_QUESTION, $quizId), ARRAY_A);
    ...
    public function save(WpAdvQuiz_Model_Category $category)
    {
    ...
    public function getCategoryArrayForImport($type = WpAdvQuiz_Model_Category::CATEGORY_TYPE_QUESTION)
    {
        $type = $type == WpAdvQuiz_Model_Category::CATEGORY_TYPE_QUESTION ? $type : WpAdvQuiz_Model_Category::CATEGORY_TYPE_QUIZ;
