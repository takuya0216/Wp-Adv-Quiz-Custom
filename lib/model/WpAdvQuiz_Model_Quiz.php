...
class WpAdvQuiz_Model_Quiz extends WpAdvQuiz_Model_Model
{
     ...
     * @return null|string|WpAdvQuiz_Model_Email
     */
    public function getAdminEmail($serialize = false)
    {
        ...
        if (is_object($this->_adminEmail) || $this->_adminEmail instanceof WpAdvQuiz_Model_Email) {
            if ($serialize) {
                return @serialize($this->_adminEmail);
            }
        } else {
            if (!$serialize) {
                if (WpAdvQuiz_Helper_Until::saveUnserialize($this->_adminEmail, $into) === false) {
                    return null;
                }
    ...
    public function getUserEmail($serialize = false)
    {
        ...
        if (is_object($this->_userEmail) || $this->_userEmail instanceof WpAdvQuiz_Model_Email) {
            if ($serialize) {
                return @serialize($this->_userEmail);
            }
        } else {
            if (!$serialize) {
                if (WpAdvQuiz_Helper_Until::saveUnserialize($this->_userEmail, $into) === false) {
                    return null;
                }
    ...
    public function getPluginContainer($serialize = false)
    {
        if ($this->_pluginContainer === null) {
            $this->_pluginContainer = new WpAdvQuiz_Model_PluginContainer();
        }

        if (is_object($this->_pluginContainer) || $this->_pluginContainer instanceof WpAdvQuiz_Model_PluginContainer) {
            if ($serialize) {
                return @serialize($this->_pluginContainer);
            }
        } else {
            if (!$serialize) {
                if (WpAdvQuiz_Helper_Until::saveUnserialize($this->_pluginContainer, $into) === false) {
                    return null;
                }
