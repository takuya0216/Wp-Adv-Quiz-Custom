...
class WpAdvQuiz_Helper_DbUpgrade
{
    ...
    public function upgrade($version)
    {
        ...
        if ($version === false || ((int)$version) > WpAdvQuiz_Helper_DbUpgrade::WPPROQUIZ_DB_VERSION) {
            $this->install();

            return WpAdvQuiz_Helper_DbUpgrade::WPPROQUIZ_DB_VERSION;
        }

        $version = (int)$version;

        if ($version === WpAdvQuiz_Helper_DbUpgrade::WPPROQUIZ_DB_VERSION) {
            return WpAdvQuiz_Helper_DbUpgrade::WPPROQUIZ_DB_VERSION;
        }
        ...
        do {
            ...
            if (method_exists($this, $f)) {
                ...
                die("WpAdvQuiz upgrade error");
            }
        } while ($version < WpAdvQuiz_Helper_DbUpgrade::WPPROQUIZ_DB_VERSION);
    ...
    {
        $this->_wpdb->query('DROP TABLE IF EXISTS `' . $this->_wpdb->prefix . 'wp_adv_quiz_category`');
        $this->_wpdb->query('DROP TABLE IF EXISTS `' . $this->_wpdb->prefix . 'wp_adv_quiz_form`');
        $this->_wpdb->query('DROP TABLE IF EXISTS `' . $this->_wpdb->prefix . 'wp_adv_quiz_lock`');
        $this->_wpdb->query('DROP TABLE IF EXISTS `' . $this->_wpdb->prefix . 'wp_adv_quiz_master`');
        $this->_wpdb->query('DROP TABLE IF EXISTS `' . $this->_wpdb->prefix . 'wp_adv_quiz_prerequisite`');
        $this->_wpdb->query('DROP TABLE IF EXISTS `' . $this->_wpdb->prefix . 'wp_adv_quiz_question`');
        $this->_wpdb->query('DROP TABLE IF EXISTS `' . $this->_wpdb->prefix . 'wp_adv_quiz_statistic`');
        $this->_wpdb->query('DROP TABLE IF EXISTS `' . $this->_wpdb->prefix . 'wp_adv_quiz_statistic_ref`');
        $this->_wpdb->query('DROP TABLE IF EXISTS `' . $this->_wpdb->prefix . 'wp_adv_quiz_template`');
        $this->_wpdb->query('DROP TABLE IF EXISTS `' . $this->_wpdb->prefix . 'wp_adv_quiz_toplist`');
    }
    ...
    public function databaseDelta()
    {
        ...
        dbDelta("
			CREATE TABLE {$this->_wpdb->prefix}wp_adv_quiz_category (
			...
			CREATE TABLE {$this->_wpdb->prefix}wp_adv_quiz_form (
			...
			CREATE TABLE {$this->_wpdb->prefix}wp_adv_quiz_lock (
			  quiz_id int(11) NOT NULL,
			...
			CREATE TABLE {$this->_wpdb->prefix}wp_adv_quiz_master (
			...
			CREATE TABLE {$this->_wpdb->prefix}wp_adv_quiz_prerequisite (
			  prerequisite_quiz_id int(11) NOT NULL,
			  quiz_id int(11) NOT NULL,
			  PRIMARY KEY  (prerequisite_quiz_id,quiz_id)
			) ENGINE=InnoDB DEFAULT CHARSET=utf8;
			
			CREATE TABLE {$this->_wpdb->prefix}wp_adv_quiz_question (
			  id int(11) NOT NULL AUTO_INCREMENT,
			  quiz_id int(11) NOT NULL,
			...
			CREATE TABLE {$this->_wpdb->prefix}wp_adv_quiz_statistic (
			...
			CREATE TABLE {$this->_wpdb->prefix}wp_adv_quiz_statistic_ref (
			...
			CREATE TABLE {$this->_wpdb->prefix}wp_adv_quiz_template (
			...
			CREATE TABLE {$this->_wpdb->prefix}wp_adv_quiz_toplist (
    ...
    private function upgradeDbV1()
    {
        ...
        $this->_wpdb->query('
			ALTER TABLE `' . $this->_wpdb->prefix . 'wp_adv_quiz_master`
    ...
    private function upgradeDbV3()
    {
        ...
        $this->_wpdb->query('
			ALTER TABLE `' . $this->_wpdb->prefix . 'wp_adv_quiz_question`
        ...
        $this->_wpdb->query('
			CREATE TABLE IF NOT EXISTS `' . $this->_wpdb->prefix . 'wp_adv_quiz_lock` (
				`quiz_id` int(11) NOT NULL,
				`lock_ip` varchar(100) NOT NULL,
				`lock_date` int(11) NOT NULL,
    ...
    private function upgradeFixDbV4()
    {
        if ($this->_wpdb->prefix != 'wp_') {
            $this->_wpdb->query('SELECT * FROM `' . $this->_wpdb->prefix . 'wp_adv_quiz_question` LIMIT 0,1');

            $names = $this->_wpdb->get_col_info('name');

            if (!in_array('tip_enabled', $names)) {
                $this->_wpdb->query('ALTER TABLE  `' . $this->_wpdb->prefix . 'wp_adv_quiz_question` ADD `tip_enabled` TINYINT( 1 ) NOT NULL AFTER  `incorrect_count`');
            }

            if (!in_array('tip_msg', $names)) {
                $this->_wpdb->query('ALTER TABLE  `' . $this->_wpdb->prefix . 'wp_adv_quiz_question` ADD `tip_msg` TEXT NOT NULL AFTER  `tip_enabled`');
            }

            if (!in_array('tip_count', $names)) {
                $this->_wpdb->query('ALTER TABLE  `' . $this->_wpdb->prefix . 'wp_adv_quiz_question` ADD  `tip_count` INT NOT NULL AFTER `tip_msg`');
            }
        }
    }
    ...
    private function upgradeDbV6()
    {
        ...
        $this->_wpdb->query('
			UPDATE `' . $this->_wpdb->prefix . 'wp_adv_quiz_question` SET `points` = 1
		');
    ...
    private function upgradeDbV7()
    {
        ...
        $this->_wpdb->query('
			ALTER TABLE  `' . $this->_wpdb->prefix . 'wp_adv_quiz_lock`
				CHANGE  `lock_ip`  `lock_ip` VARCHAR( 100 ) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL
		');

        $this->_wpdb->query('
			ALTER TABLE  `' . $this->_wpdb->prefix . 'wp_adv_quiz_lock` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci
		');

        $this->_wpdb->query('
			ALTER TABLE  `' . $this->_wpdb->prefix . 'wp_adv_quiz_master` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci
		');

        $this->_wpdb->query('
			ALTER TABLE  `' . $this->_wpdb->prefix . 'wp_adv_quiz_question` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci
		');
    ...
    {
        if ($this->_wpdb->prefix != 'wp_') {
            $this->_wpdb->query('SELECT * FROM `' . $this->_wpdb->prefix . 'wp_adv_quiz_master` LIMIT 0,1');
    ...
    private function upgradeDbV9()
    {
        ...
        $this->_wpdb->query('
			TRUNCATE `' . $this->_wpdb->prefix . 'wp_adv_quiz_lock`
		');
        ...
        $this->_wpdb->query('
			CREATE TABLE IF NOT EXISTS `' . $this->_wpdb->prefix . 'wp_adv_quiz_statistic` (
				  `quiz_id` int(11) NOT NULL,
				  `question_id` int(11) NOT NULL,
				  `user_id` bigint(20) unsigned NOT NULL,
				  `correct_count` int(10) unsigned NOT NULL,
        ...
        $this->_wpdb->query('
			INSERT INTO `' . $this->_wpdb->prefix . 'wp_adv_quiz_statistic` (quiz_id, question_id, user_id, correct_count, incorrect_count, hint_count)
				SELECT
					question.quiz_id, id, 0, question.correct_count, question.incorrect_count, tip_count
				FROM 
					`' . $this->_wpdb->prefix . 'wp_adv_quiz_question` as question
				WHERE
    ...
    private function upgradeDbV11()
    {
        ...
        $this->_wpdb->query('
			ALTER TABLE `' . $this->_wpdb->prefix . 'wp_adv_quiz_statistic`
				ADD  `correct_answer_count` INT UNSIGNED NOT NULL
		');

        $this->_wpdb->query('UPDATE `' . $this->_wpdb->prefix . 'wp_adv_quiz_statistic` SET `correct_answer_count` = `correct_count`');

        $this->_wpdb->query('UPDATE `' . $this->_wpdb->prefix . 'wp_adv_quiz_question` SET `points_answer` = `points`');
    ...
    private function upgradeDbV13()
    {
        ...
        if (!in_array('hide_answer_message_box', $names)) {
            $this->_wpdb->query('ALTER TABLE  `' . $this->_wpdb->prefix . 'wp_adv_quiz_master` ADD  `hide_answer_message_box` TINYINT( 1 ) NOT NULL');
        }

        if (!in_array('disabled_answer_mark', $names)) {
            $this->_wpdb->query('ALTER TABLE  `' . $this->_wpdb->prefix . 'wp_adv_quiz_master` ADD  `disabled_answer_mark` TINYINT( 1 ) NOT NULL');
        }

        if (!in_array('show_max_question', $names)) {
            $this->_wpdb->query('ALTER TABLE  `' . $this->_wpdb->prefix . 'wp_adv_quiz_master` ADD  `show_max_question` TINYINT( 1 ) NOT NULL');
        }

        if (!in_array('show_max_question_value', $names)) {
            $this->_wpdb->query('ALTER TABLE  `' . $this->_wpdb->prefix . 'wp_adv_quiz_master` ADD  `show_max_question_value` INT UNSIGNED NOT NULL');
        }

        if (!in_array('show_max_question_percent', $names)) {
            $this->_wpdb->query('ALTER TABLE  `' . $this->_wpdb->prefix . 'wp_adv_quiz_master` ADD  `show_max_question_percent` TINYINT( 1 ) NOT NULL');
        }
    ...
    private function upgradeDbV15()
    {
        ...
        $this->_wpdb->query('
			CREATE TABLE IF NOT EXISTS `' . $this->_wpdb->prefix . 'wp_adv_quiz_prerequisite` (
			 	`prerequisite_quiz_id` int(11) NOT NULL, 
			 	`quiz_id` int(11) NOT NULL, 
			  	PRIMARY KEY (`prerequisite_quiz_id`,`quiz_id`) 
			) ENGINE=InnoDB DEFAULT CHARSET=utf8;
        ...
        $this->_wpdb->query('
			CREATE TABLE IF NOT EXISTS `' . $this->_wpdb->prefix . 'wp_adv_quiz_toplist` (
				  `toplist_id` int(11) NOT NULL AUTO_INCREMENT,
				  `quiz_id` int(11) NOT NULL,
				  `date` int(10) unsigned NOT NULL,
				  `user_id` bigint(20) unsigned NOT NULL,
        ...
        $results = $this->_wpdb->get_results('SELECT id, answer_type, answer_json, points_per_answer, points_answer  FROM `' . $this->_wpdb->prefix . 'wp_adv_quiz_question`',
            ARRAY_A);
        ...
        foreach ($results as $row) {
            ...
            if ($row['answer_type'] == 'single' || $row['answer_type'] == 'multiple') {
                foreach ($data['classic_answer']['answer'] as $k => $v) {
                    $x = new WpAdvQuiz_Model_AnswerTypes();
            ...
            $this->_wpdb->update(
                $this->_wpdb->prefix . 'wp_adv_quiz_question',
                array(
        ...
        $this->_wpdb->query(
            'UPDATE ' . $this->_wpdb->prefix . 'wp_adv_quiz_question
			SET
        ...
        $this->_wpdb->query(
            'UPDATE
				' . $this->_wpdb->prefix . 'wp_adv_quiz_statistic AS s
			SET
				s.points = ( SELECT q.points_answer FROM ' . $this->_wpdb->prefix . 'wp_adv_quiz_question AS q WHERE q.id = s.question_id ) * s.correct_answer_count
			WHERE
    ...
    private function upgradeDbV17()
    {
        ...
        $this->_wpdb->query('
			CREATE TABLE IF NOT EXISTS `' . $this->_wpdb->prefix . 'wp_adv_quiz_category` (
			  `category_id` int(10) unsigned NOT NULL AUTO_INCREMENT,
			  `category_name` varchar(200) NOT NULL,
			  PRIMARY KEY (`category_id`)
			) ENGINE=InnoDB  DEFAULT CHARSET=utf8;
        ...
        $this->_wpdb->update($this->_wpdb->prefix . 'wp_adv_quiz_master',
            array(
                'quiz_modus' => WpAdvQuiz_Model_Quiz::QUIZ_MODUS_SINGLE,
                'back_button' => 0,
                'check_answer' => 0
            ),
        ...
        $this->_wpdb->update($this->_wpdb->prefix . 'wp_adv_quiz_master',
            array(
                'quiz_modus' => WpAdvQuiz_Model_Quiz::QUIZ_MODUS_CHECK,
                'back_button' => 0
            ),
            array('check_answer' => 1));

        $this->_wpdb->update($this->_wpdb->prefix . 'wp_adv_quiz_master',
            array('quiz_modus' => WpAdvQuiz_Model_Quiz::QUIZ_MODUS_BACK_BUTTON),
            array('back_button' => 1));
    ...
    private function upgradeDbV18()
    {
        ...
        $this->_wpdb->query('
			DELETE s
				FROM ' . $this->_wpdb->prefix . 'wp_adv_quiz_statistic AS s
					LEFT JOIN ' . $this->_wpdb->prefix . 'wp_adv_quiz_master AS m ON ( s.quiz_id = m.id )
					LEFT JOIN ' . $this->_wpdb->prefix . 'wp_adv_quiz_question AS q ON ( s.question_id = q.id )
			WHERE m.id IS NULL OR q.id IS NULL
		');

        //Start - Update Statistic
        $this->_wpdb->query('
			CREATE TABLE IF NOT EXISTS ' . $this->_wpdb->prefix . 'wp_adv_quiz_statistic_ref (
			  `statistic_ref_id` int(10) unsigned NOT NULL AUTO_INCREMENT,
			  `quiz_id` int(11) NOT NULL,
			  `user_id` bigint(20) unsigned NOT NULL,
			  `create_time` int(11) NOT NULL,
        ...
        $this->_wpdb->query('
			INSERT INTO ' . $this->_wpdb->prefix . 'wp_adv_quiz_statistic_ref
        ...
        $this->_wpdb->query('
			UPDATE ' . $this->_wpdb->prefix . 'wp_adv_quiz_statistic AS s
				LEFT JOIN ' . $this->_wpdb->prefix . 'wp_adv_quiz_statistic_ref AS sf
        ...
        if (in_array('check_answer', $names)) {
            $this->_wpdb->query('ALTER TABLE  `' . $this->_wpdb->prefix . 'wp_adv_quiz_master` DROP `check_answer` ');
        }

        if (in_array('back_button', $names)) {
            $this->_wpdb->query('ALTER TABLE  `' . $this->_wpdb->prefix . 'wp_adv_quiz_master` DROP `back_button` ');
        }

        if (in_array('question_on_single_page', $names)) {
            $this->_wpdb->query('ALTER TABLE  `' . $this->_wpdb->prefix . 'wp_adv_quiz_master` DROP `question_on_single_page` ');
        }
    ...
    private function upgradeDbV20()
    {
        ...
        if (!in_array('autostart', $names)) {
            $this->_wpdb->query('ALTER TABLE  `' . $this->_wpdb->prefix . 'wp_adv_quiz_master`
									ADD  `autostart` TINYINT( 1 ) UNSIGNED NOT NULL DEFAULT  \'0\' ');
        }
        ...
        if (!in_array('answer_points_diff_modus_activated', $names)) {
            $this->_wpdb->query('ALTER TABLE  `' . $this->_wpdb->prefix . 'wp_adv_quiz_question`
									ADD `answer_points_diff_modus_activated` TINYINT( 1 ) UNSIGNED NOT NULL ');
        }
    ...
    private function upgradeDbV21()
    {
        ...
        $this->_wpdb->query('
			ALTER TABLE  ' . $this->_wpdb->prefix . 'wp_adv_quiz_statistic_ref
        ...
        $this->_wpdb->query('UPDATE ' . $this->_wpdb->prefix . 'wp_adv_quiz_question
				SET online = 1');
    ...
    {
        $this->_wpdb->query('
			ALTER TABLE  ' . $this->_wpdb->prefix . 'wp_adv_quiz_category
        ...
        $this->_wpdb->query('UPDATE ' . $this->_wpdb->prefix . 'wp_adv_quiz_master
				SET category_id = 0');

        $this->_wpdb->query('UPDATE ' . $this->_wpdb->prefix . 'wp_adv_quiz_category
				SET type = \'QUESTION\'');
    ...
    private function upgradeDbV23()
    {
        ...
        $adminEmailNew = new WpAdvQuiz_Model_Email();
        ...
        $userEmailNew = new WpAdvQuiz_Model_Email();
    ...
    {
        $this->_wpdb->query('
			ALTER TABLE  ' . $this->_wpdb->prefix . 'wp_adv_quiz_form
        ...
        $this->_wpdb->query('UPDATE ' . $this->_wpdb->prefix . 'wp_adv_quiz_statistic
				SET solved_count = -1');
