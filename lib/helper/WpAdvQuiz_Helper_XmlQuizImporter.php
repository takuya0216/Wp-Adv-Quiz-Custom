...
class WpAdvQuiz_Helper_XmlQuizImporter implements WpAdvQuiz_Helper_QuizImporterInterface
{
    /**
     * @var string
     */
    ...
    public function getImport()
    {
        ...
        if ($xml === false) {
            return new WP_Error(__('XML could not be loaded.', 'wp-adv-quiz'));
        }
    ...
    public function import($ids = false)
    {
        ...
        foreach ($data['master'] as $quiz) {
            ...
            if (get_class($quiz) !== 'WpAdvQuiz_Model_Quiz') {
                continue;
            }
    ...
    /**
     * @param SimpleXMLElement $xml
     * @return null|WpAdvQuiz_Model_Quiz
     */
    private function createQuizModel($xml)
    {
        $model = new WpAdvQuiz_Model_Quiz();
        ...
        if (isset($xml->adminEmail)) {
            $adminEmail = new WpAdvQuiz_Model_Email();
        ...
        if (isset($xml->userEmail)) {
            $userEmail = new WpAdvQuiz_Model_Email();
     ...
     * @return NULL|WpAdvQuiz_Model_Question
     */
    private function createQuestionModel($xml)
    {
        $model = new WpAdvQuiz_Model_Question();
        ...
        if (isset($xml->answers)) {
            foreach ($xml->answers->answer as $answer) {
                $answerModel = new WpAdvQuiz_Model_AnswerTypes();
