...
class WpAdvQuiz_Helper_XmlQuizExporter implements WpAdvQuiz_Helper_QuizExporterInterface
{
    ...
    {
        return 'WpAdvQuiz_export_' . time() . '.xml';
    }
    ...
    public function export($ids)
    {
        ...
        $root = $dom->createElement('wpAdvQuiz');

        $dom->appendChild($root);

        $header = $dom->createElement('header');
        $header->setAttribute('version', WPADVQUIZ_VERSION);
    ...
    private function getQuizElement($dom, $quiz, $forms)
    {
        ...
        if ($quiz->getAdminEmail() !== null) {
            /* @var $adminEmail WpAdvQuiz_Model_Email */
    ...
    /**
     * @param DOMDocument $dom
     * @param WpAdvQuiz_Model_Question $question
     * @return DOMDocument
     */
    private function createQuestionElement($dom, $question)
    {
        ...
        if (is_array($answerData)) {
            foreach ($answerData as $answer) {
                /** @var WpAdvQuiz_Model_AnswerTypes $answer */
